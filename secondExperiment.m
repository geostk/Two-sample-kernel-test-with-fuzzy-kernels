function [STAT1,STAT2]=secondExperiment(alph,m,shuff,nTest,nroEpoch,XX,test)
% experiment p different than q
%  only gaussian RBF kernel with kernel parameter computed using the
% median heuristic and crisp dataset
% input :
% 
% %test level
% alph=0.5;
...
for epoch= 1:nroEpoch
    for i=1:nTest        %cho
        ...
        [lambda, ~ ,~]=computeLambda(XX(ind1,2:10),XX(ind2,2:10));
        %XX
        %---------------------------------------------
        K=RBF_kernel(XX(ind1,2:10),XX(ind1,2:10),1/lambda);
        
        %YY
        %---------------------------------------------
        L=RBF_kernel(XX(ind2,2:10),XX(ind2,2:10),1/lambda);
        
        %XY
        %---------------------------------------------
        KL=RBF_kernel(XX(ind1,2:10),XX(ind2,2:10),1/lambda);
