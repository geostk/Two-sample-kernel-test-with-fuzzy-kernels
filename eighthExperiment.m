function [STAT1,STAT2]=eighthExperiment(alph,m,shuff,nTest,nroEpoch,XX,test)
% two sample kernel test
% each dimension is considered with a linear kernel 
% input :
% 
% %test level
% alph=0.5;
...
for epoch= 1:nroEpoch
    ...
    for i=1:nTest
        ...
        K=0;
        for j=2:10           
            K=K+linearKernel(XX(ind1,j),XX(ind1,j));
        end
        ...
        L=0;
        for j=2:10            
            L=L+linearKernel(XX(ind2,j),XX(ind2,j));
        end
        ...
        KL=0;
        for j=2:10            
            KL=KL+linearKernel(XX(ind1,j),XX(ind2,j));
        end
